...
const App = () => {
  ...
  const addGoalHandler = (text) => {
    const newGoal = {
      id: Math.random().toString(), // DB Table 의 PK로 처리가 될 것.
      text,
    };
    // 상태변수(배열) 변경
  ...
  // 삭제 이벤트 핸들러를 CourseItem 까지 내려보내야 함.
  // 결국 삭제라는 것은 배열의 상태가 변한다는 거고, 상태는 App.js가 관리하는 중.
  const deleteGoalHandler = (targetId) => {
    setGoals(goals.filter((goal) => goal.id !== targetId));
    // console.log('선택된 목표 id:' + id);
