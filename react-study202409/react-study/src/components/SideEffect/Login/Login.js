...
const formReducer = (state, action) => {
  console.log('eamil reducer call');
...
const Login = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    // 의존성 배열에 상태변수를 넣어주면 그 상태 변수가 변경될 때마다 useEffec가 재실행됨
  }, [emailState.vlaue, pwState.vlaue, emailState.isValid, pwState.isValid]);
      ...
      <form onSubmit={submitHandle}>
        ...
        <div className={styles.actions}>
          <ButtonComponent
            ...
            disabled={!FormState.formIsValid} // button은 disabled를 통해 비활성화 기능 구현. true면 비활성, false면 활성
          >
            Login
          </ButtonComponent>
        </div>
