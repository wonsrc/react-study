...
// state: 업데이트 전의 최신 상태
// action: 어떤 업데이트를 하는지에 대한 정보와 필요값들이 들어있음.(dispatch에 의해 전달)
const cartReducer = (state, action) => {
  if (action.type === 'ADD') {
    // 기존 카트에 이미 등록된 메뉴인지 아닌지의 여부부터 확인해야 할것 같음!
    // findIndex를 사용해서, 새롭게 추가할 item의 id와 기존 상품의 id를 비교해서 idx 찾기.
    const idx = state.items.findIndex(
      (oldItem) => action.item.id === oldItem.id,
    );
    // 기존 카트 아이템 -> 마지막에 최신 배열 상태를 리턴해야함
    // 기존 상태 배열을 복사해서 조작한 후에 마지막에 updateItems를
    const existingItems = [...state.items]; // 기존 배열을 복사
    const prevCartItem = existingItems[idx];
    let updateItems;
...
// App.js 에서 CartContext를 바로 provide 할수도 있지만,
